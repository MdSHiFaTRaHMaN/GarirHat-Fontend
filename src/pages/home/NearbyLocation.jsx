...
const NearbyLocation = () => {
  const { alDivition, isLoading } = useAlDivition(); // Get all locations from API
  ...
  const handleLocation = (location) => {
    ...
    const newSearchParams = new URLSearchParams(searchParams); // Get current search params
    const currentMakes = newSearchParams.getAll("make"); // Get all makes

    // Avoid duplicates
    if (!currentMakes.includes(location)) {
      newSearchParams.append("make", location); // Append new make
    }
...
//     handleClose();
  ...
  return (
    <div
      ...
      style={{ backgroundImage: `url(${CityBackground})` }} // Background Image
    >
